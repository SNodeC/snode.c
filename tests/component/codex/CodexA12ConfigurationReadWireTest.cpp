...
namespace {
    ...
    codex::Json initializeResult() {
        return {
            {"codexHome", "/tmp/codex-a1-2-configuration-read-wire"},
            {"platformFamily", "unix"},
            {"platformOs", "linux"},
            {"userAgent", "codex-a1-2-configuration-read-wire/1"},
        };
    }

    codex::Json configReadResult() {
        return {
            {"config", {{"analytics", {{"enabled", true}}}, {"web_search", "cached"}, {"futureConfig", {{"retained", true}}}}},
            {"layers", nullptr},
    ...
    codex::Json configRequirementsReadResult() {
        return {
            {"requirements",
             {{"allowAppshots", true}, {"allowRemoteControl", nullptr}, {"allowedWebSearchModes", codex::Json::array({"cached"})}}},
            {"futureResultField", true},
        };
    }
        ...
        bool send(std::string message) {
            ...
            if (replyMode == ReplyMode::RemoteError) {
                return injectCurrent({{"id", envelope.at("id")},
                                      {"error",
                                       {{"code", -32'414},
                                        {"message", "synthetic configuration remote failure"},
    ...
    public:
        explicit TestClient(const std::shared_ptr<UnixTransportState>& state)
            : AppServerClient(std::make_unique<UnixTranscriptTransport>(state),
                              {"codex_a1_2_configuration_read_wire_test", "Codex A1.2 Configuration Read Wire Test", "1"}) {
        }
    };
    ...
    class ConfigurationReadWireRunner {
    public:
        explicit ConfigurationReadWireRunner(tests::support::TestResult& result)
            : result(result)
        ...
        void start() {
            expect(socketReady, "A1.2 configuration-read wire test opens its AF_UNIX socketpair");
            if (!socketReady) {
                finished = true;
                core::SNodeC::stop();
                return;
            ...
            client->setOnStateChanged([this](const codex::StateChange& change) {
                if (change.current == codex::State::Ready) {
                    ...
                    expect(false, "A1.2 configuration-read socket transcript connection must not fail");
                    client->stop();
    ...
    private:
        ...
        std::vector<std::string> expectedMethods() const {
            return {"config/read", "configRequirements/read"};
        }
        ...
        std::function<void(const typed::OperationResult<Result>&)> handler(std::string method, codex::Json expectedResult, Phase phase) {
            return [this, method = std::move(method), expectedResult = std::move(expectedResult), phase](
                ...
                if (phase == Phase::Success || phase == Phase::Generation) {
                    ...
                    if (phase == Phase::Success && method == "config/read") {
                        throw std::runtime_error("intentional configuration completion callback failure");
                    }
                    return;
                }
                if (phase == Phase::RemoteError) {
                    ...
                    expect(operation.kind == typed::OperationResult<Result>::Kind::RemoteError && !operation.value &&
                               ...
                               operation.remoteError->message == "synthetic configuration remote failure" &&
        ...
        void buildCases() {
            ...
            cases.push_back(makeOperation<typed::ConfigReadResponse>("config/read",
                                                                     ...
                                                                     {{"cwd", nullptr}, {"includeLayers", false}},
                                                                     configReadResult(),
                                                                     [&configuration](auto params, auto resultHandler) {
                                                                         return configuration.read(std::move(params),
                                                                                                   std::move(resultHandler));
                                                                     }));
            cases.push_back(makeOperation<typed::ConfigRequirementsReadResponse>("configRequirements/read",
                                                                                 detail::ClientRequestTarget::ConfigRequirementsRead,
                                                                                 typed::Unit{},
                                                                                 nullptr,
        ...
        void invokeLocalRejections() {
            ...
            for (OperationCase& operation : cases) {
                ...
                const bool registryExact = entry.key == expectedKey && registeredTarget != nullptr && *registeredTarget == operation.target;
                const bool rejected =
                ...
                expect(registryExact, operation.method + " dispatch starts from its exact canonical registry identity and target");
            }
            expect(exact == OperationCount && unexpectedLocalCallbacks == 0,
                   "both configuration-read operations reject locally without callbacks or transport traffic");
        }
        ...
        void invokeBatch(Phase phase) {
            ...
            expect(state->outbound.size() == before + OperationCount,
                   "both configuration-read operations cross the AF_UNIX transport exactly once");
        ...
        void completeSuccess() {
            ...
            expect(successOrder == methods && successCallbacks == OperationCount,
                   "both configuration-read success callbacks complete asynchronously in submission order");
            ...
            for (const std::string& method : methods) {
                ...
                expect(typedObserverCounts[method] == 1 && rawObserverCounts[method] == 1,
                       method + " coexists with one typed and one raw configWarning observer");
            }
            expect(observerOrder == expectedObserverOrder,
                   "typed configWarning observers precede raw observers and callback exceptions are contained");
        ...
        void beginCancellation() {
            expect(remoteOrder == expectedMethods(), "both configuration-read remote errors retain callback order");
        ...
        void maybeRestart() {
            if (!restartScheduled && firstStopObserved && cancellationCallbacks == OperationCount) {
                ...
                expect(cancellationOrder == expectedMethods(),
                       "both held configuration-read operations cancel exactly once in submission order");
                core::EventReceiver::atNextTick([this]() {
                    client->start();
                });
            }
        ...
        void beginGeneration() {
            ...
            core::EventReceiver::atNextTick([this]() {
                expect(generationCallbacks == 0, "stale-generation configuration-read responses are rejected before current delivery");
        ...
        void completeGeneration() {
            expect(generationCallbacks == OperationCount && generationOrder == expectedMethods(),
                   "both current-generation configuration-read responses complete once in callback order");
            client->stop();
        }
...
int main(int argc, char* argv[]) {
    ...
    ConfigurationReadWireRunner runner(result);
    runner.start();
    ...
    result.expectTrue(!timedOut && runner.isFinished(), "A1.2 configuration-read AF_UNIX lifecycle matrix completes before the watchdog");
    result.expectEqual(0, startResult, "A1.2 configuration-read AF_UNIX lifecycle matrix stops the event loop cleanly");
    core::SNodeC::free();
