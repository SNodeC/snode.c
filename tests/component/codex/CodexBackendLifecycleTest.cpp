...
namespace {
    ...
    typed::Turn turn(const std::string& id, typed::TurnStatus status) {
        ...
        value.threadId = typed::ThreadId{"codex-lifecycle-thread"};
...
int main() {
    ...
    const std::filesystem::path path = std::filesystem::temp_directory_path() / "snodec-codex-backend-lifecycle.jsonl";
...

    {
        ...
        applyTyped(typed::Event{typed::ThreadStarted{thread("codex-lifecycle-thread"), ai::openai::codex::Json::object()}});
    ...
    result.expectTrue(countMessage(captured, "thread created: thread=codex-lifecycle-thread") == 1, "thread creation is emitted exactly once");
    ...
    result.expectTrue(countMessage(captured, "turn started: thread=codex-lifecycle-thread turn=turn-hydrated") == 0,
                      "generic turn hydration does not fabricate a start");
    result.expectTrue(countMessage(captured, "turn started: thread=codex-lifecycle-thread turn=turn-success") == 1, "successful turn has one start");
    result.expectTrue(countMessage(captured, "turn completed: thread=codex-lifecycle-thread turn=turn-success") == 1,
                      "successful turn has one completion");
    result.expectTrue(countMessage(captured, "turn failed: thread=codex-lifecycle-thread turn=turn-failed") == 1,
                      "failed turn has one failure and no later completion");
    result.expectTrue(countMessage(captured, "turn completed: thread=codex-lifecycle-thread turn=turn-failed") == 0,
                      "failed turn cannot later complete");
    result.expectTrue(countMessage(captured, "turn cancelled: thread=codex-lifecycle-thread turn=turn-cancelled") == 1,
                      "cancelled turn has one cancellation");
    result.expectTrue(countMessage(captured, "turn completed: thread=codex-lifecycle-thread turn=turn-cancelled") == 0,
