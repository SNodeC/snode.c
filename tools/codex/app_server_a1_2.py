...
def build_reports(
    ...
    for key in keys:
        ...
        registry_row = baseline_row["registry"]
    ...
    plan = {
        ...
        "codex_version": CODEX_VERSION,
        "authority_note": (
            "Review/batching evidence only. Frozen starting runtime/status "
            "facts are copied from the sole canonical production registry; "
            "this report is not a runtime registry or dispatch input."
        ),
        "sources": sources,
        ...
        "counts": {
            ...
            "identity_start_mapping_sha256": sha256_json(
...
def report_diagnostics(
    ...
    if dict(sorted(statuses.items())) != EXPECTED_START_SCHEMA_STATUS:
        add(
            "StartStateMismatch",
            "$.identities[*].current_schema_status",
            "initial schema-status split changed",
        )
    if (
        dict(sorted(implementation_statuses.items()))
        != EXPECTED_START_IMPLEMENTATION_STATUS
    ):
        add(
            "StartStateMismatch",
            "$.identities[*].current_implementation_status",
            "initial implementation-status split changed",
        )
    ...
    if (
        start_mapping_sha256
        != EXPECTED_IDENTITY_START_MAPPING_SHA256
        or counts.get("identity_start_mapping_sha256")
        != EXPECTED_IDENTITY_START_MAPPING_SHA256
    ):
        add(
            "PlanStartStateMappingMismatch",
            "$.identities",
            "per-identity frozen runtime/status/completeness mapping "
            "differs from the exact reviewed start state",
        )
    try:
