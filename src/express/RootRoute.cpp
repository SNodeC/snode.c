...
#define DEFINE_ROOTROUTE_REQUESTMETHOD(METHOD, HTTP_METHOD)                                                                                \
    ...
    Route& RootRoute::METHOD(const std::function<void(const std::shared_ptr<Request>& req, const std::shared_ptr<Response>& res)>& lambda) \
        const {                                                                                                                            \
...
namespace express {
    ...
    void RootRoute::dispatch(const std::shared_ptr<web::http::server::Request>& req,
                             const std::shared_ptr<web::http::server::Response>& res) {
    ...
    void RootRoute::dispatch(Controller& controller) {
        if (!Route::dispatch(controller, "")) {
            ...
            } else if (controller.laxRouting() && !Route::dispatch(controller, "")) {
                controller.getResponse()->sendStatus(404);
            }
        }
    }
