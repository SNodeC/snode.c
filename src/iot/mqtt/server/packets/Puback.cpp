...
namespace iot::mqtt::packets::deserializer {
    ...
    Puback::Puback(uint32_t remainingLength, uint8_t flags)
        : iot::mqtt::ControlPacketDeserializer(remainingLength, flags, MQTT_PUBACK_FLAGS) {
        this->flags = flags;
    }
    ...
    void Puback::propagateEvent(iot::mqtt::SocketContext* socketContext) {
        dynamic_cast<iot::mqtt::server::SocketContext*>(socketContext)->_onPuback(*this);
    }
