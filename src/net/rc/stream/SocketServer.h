...
namespace net::rc::stream {
    ...
    public:
        ...
        void listen(uint8_t channel, const std::function<void(const core::ProgressLog&)>& onError) const {
            Super::getConfig().Local::setChannel(channel);

            listen(onError);
        }

        void listen(uint8_t channel, int backlog, const std::function<void(const core::ProgressLog&)>& onError) const {
            Super::getConfig().Local::setChannel(channel);
        ...
        void listen(const std::string& btAddress, uint8_t channel, const std::function<void(const core::ProgressLog&)>& onError) const {
