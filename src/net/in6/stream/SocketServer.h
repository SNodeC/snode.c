...
namespace net::in6::stream {
    ...
    public:
        ...
        void listen(const std::string& ipOrHostname, uint16_t port, const std::function<void(const SocketAddress&, int)>& onError) const {
            Super::getConfig().Local::setIpOrHostname(ipOrHostname).Local::setPort(port);
        ...
        void listen(const std::string& ipOrHostname,
            ...
            Super::getConfig().Local::setIpOrHostname(ipOrHostname).Local::setPort(port).setBacklog(backlog);
