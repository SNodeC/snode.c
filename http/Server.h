...
namespace http {
    ...
    public:
        void listen(in_port_t port, const std::function<void(int err)>& onError) {
            errno = 0;

            socketServer.listen(port, 5, onError);
        }

        void listen(const std::string host, in_port_t port, const std::function<void(int err)>& onError) {
            errno = 0;
