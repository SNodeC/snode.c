...
## PR #152 cleanup after production-threshold repair

PR #152 is a pure Migration 1 PR. PR #151 is already merged into `master` and provides the production-threshold repair (`0aa69f6f0e5b77c10fdb0b0a10384f3b78717a92`). This PR depends on the already-merged PR #151 production-threshold repair and verifies compatibility with it; this PR does not implement the PR #151 repair again.

This PR is based on `master` after PR #151. The branch was rebased onto current repaired `master`, and the final PR diff was cleaned so it contains only the four Migration 1 files listed below.

PR #151 repair files are present on the base branch and are not changed by this PR:

- `docs/logging/semantic-production-threshold-repair-report.md`
- `tests/unit/log/SemanticProductionThresholdRepairTest.cpp`

Repair-compatibility confirmations:
...
- This PR does not modify `SemanticLogger.*`, `LogScopeOwner.*`, `ConfigInstance.cpp`, `SocketConnection.cpp`, `SocketContext.cpp`, `SocketServer.h`, `SocketClient.h`, `SocketConnector.h`, `SocketAcceptor.h`, or `SemanticProductionThresholdRepairTest.cpp`.
...
Post-cleanup inventory result: 6 deferred legacy macro call sites remain in free helper functions only (lines 67, 70, 74, 90, 93, and 97); no `VLOG` call sites exist in the file.
