...
A final connection-scope audit reviewed 246 identity-bearing production record sites across the Phase 3 surface. At the reviewed head, 98 connection-bound records were compliant: 95 previously counted records (51 formerly `BOUND_DUPLICATE` and 44 formerly `STATIC_BUT_BINDABLE` MQTT records) plus three bound missing-request diagnostics in `Response.cpp`. There are zero residual `BOUND_DUPLICATE`; the remaining classification is 38 `STATIC_BUT_BINDABLE`, 12 `PREBOUND_FALLBACK` call sites, 14 `DOMAIN_IDENTITY`, 74 `GLOBAL_COMPONENT_DIAGNOSTIC`, and 10 `FROZEN_PHASE1_PHASE2`. The identity correction changed all 38 bindable emissions: 13 connected HTTP server-upgrade records in `Response.cpp`, 17 HTTP client upgrade/SSE-response records in `Request.cpp`, and eight EventSource connection/request/stream diagnostics. The three missing-request diagnostics bring the connected `Response.cpp` total to 16. Thirty of the 38 corrections removed a textual identity prefix; eight adjacent protocol/subprotocol detail emissions had no textual prefix but were moved with their operation to the same bound logger. That audit found no MQTT, Codex, MariaDB, or other non-HTTP production change; the exhaustive parameterless-helper closure below subsequently found and corrected two WebSocket subprotocol summaries.
...
The HTTP server record is specifically a `PREBOUND_FALLBACK` with a post-disconnect subtype: `Response::isConnected()` is equivalent to `socketContext != nullptr`, and `Response::disconnect()` clears that pointer. The disconnected `Response::upgrade()` branch therefore intentionally uses the parameterless HTTP server logger, carries no fabricated connection identity, and now emits `Unexpected disconnect during upgrade` after removal of the redundant `Upgrade:` label. The connected missing-request path remains connection-bound but now returns before any request-derived access: it emits `Upgrade request has gone away`, sets `Connection: close` and HTTP 500, invokes the status callback exactly once with an empty selected protocol, and performs no factory selection or context switch. This is a narrow HTTP correctness correction discovered during the identity audit; normal upgrade behavior is unchanged. It adds three bound diagnostic sites, increasing the reviewed total from 243 to 246; the fallback count does not change.

### Parameterless-helper closure

The closure audit lexically inspected every literal empty call to the discovered Phase 3 semantic helpers under HTTP, WebSocket, MQTT, Codex, and MariaDB production code. It found 83 calls before correction. The two `SubProtocol::detach()` payload-total summaries were `UNRESOLVED_STATIC_BUT_BINDABLE`; both now reuse the same connection-bound `webSocketSubProtocolLog` as `subprotocol stopped`. The remaining 81 calls are intentionally parameterless and appear individually, with a stable source marker, classification, and reason, in `Phase3ParameterlessSemanticLoggerPolicyTest`. That machine-readable allowlist is the authoritative per-call-site classification artifact; the table below is its human-readable summary.
...
| Codex reducer | local `lifecycleLog` | 4 | `DOMAIN_OR_PROTOCOL_SCOPE` | authoritative typed thread/turn lifecycle events keyed by domain IDs |
...
The classification totals are 21 `PREBOUND_FALLBACK`, five `POSTDISCONNECT_FALLBACK`, 26 `GLOBAL_COMPONENT_DIAGNOSTIC`, 22 `DOMAIN_OR_PROTOCOL_SCOPE`, four `FROZEN_PHASE1_PHASE2`, three `HELPER_FALLBACK_IMPLEMENTATION`, and zero `APPLICATION_FACING` or `UNRESOLVED_STATIC_BUT_BINDABLE`. Object-bound `log()`/`frameworkLog()` calls, semantic helpers with nonempty arguments, and helper definitions are deliberately excluded. Domain identities—including MQTT client ID/topic/packet/QoS fields, HTTP request and upgrade data, Codex thread/turn IDs, and database operation data—remain in their messages. This closure changes no lifecycle or protocol behavior and introduces no API, logger owner/cache, persistent logger state, or class-layout change.
...
The direct Phase 3 lifecycle policy, parameterless-helper policy, Codex reducer, fake-transport AppServerClient, MQTT lifecycle, and HTTP streaming-source failure tests passed 6/6. The parameterless-helper policy discovers and allowlists all 81 remaining calls and contains negative self-tests proving that an injected unexpected call, a stale or removed call, duplicate entries, missing classification/reason, and an unresolved bindable classification all make the guard fail. The MQTT test captures a common packet diagnostic and a server-role diagnostic after connection binding, checks their structured instance/connection/component/role fields and retained domain payload, and verifies that text formatting places architectural identity before the formatter separator while the payload begins with the diagnostic. The real in-process WebSocket close-handshake test now also verifies that both subprotocol payload-total summaries carry the same structured instance and connection identity as the stop record. The final HTTP closure retains the real upgrade test's connection-bound metadata/payload assertions and formatter assertion using the existing bound HTTP server logger. The Phase 3 tests otherwise continue to use structured semantic JSON for vocabulary, level, count, ordering, and available semantic identity fields.

The streaming-source regression sends the complete declared seven-byte response body, reports a source error, then drives later EOF and normal context-detach cleanup. It observes exactly one `request started` and one Debug-level `request failed` with matching bound connection identity, zero `request completed`, and zero `request aborted`; it also verifies connection close, retained response-error diagnostics, and no duplicate terminal during late cleanup. This is the focused proof for the private `sourceFailed` behavior.

Additional GCC results were:

- all logging tests: 46/46 passed;
- Phase 1/2 context, listener, and transport policy tests: 4/4 passed;
- HTTP tests: 29/29 passed;
...
- complete CTest: 253 passed, zero failed, and one skipped out of 254.
...
For the missing-upgrade-request correction, GCC 15.3 and Clang 21.1.8 rebuilt the changed HTTP server and focused tests, and the policy, HTTP regression, and in-process WebSocket upgrade tests passed 3/3 with each compiler. The focused null-request regression also passed 1/1 under AddressSanitizer/LeakSanitizer with leak detection enabled and 1/1 under UndefinedBehaviorSanitizer. The UBSan build excluded `vptr` because the repository's existing cross-DSO RTTI and `--no-undefined` combination does not link with that check; null, alignment, bounds, and the other undefined-behavior checks remained enabled. Neither sanitizer reported a finding.

For the parameterless-helper closure, Clang 21.1.8 rebuilt `Phase3ParameterlessSemanticLoggerPolicyTest`, `Phase3HttpStreamingSourceFailureTest`, `InetWebSocketServerClientCloseHandshakeTest`, and their affected HTTP/WebSocket libraries; the three tests passed 3/3. The WebSocket test was run in the unrestricted local-loopback environment because socket bind/connect is blocked by the sandbox. The focused configuration retains the three compatibility demotions listed above; no closure-source compile error was reported.
...
The exact-length streaming-source failure regression passed 1/1 under AddressSanitizer/LeakSanitizer with `detect_leaks=1:halt_on_error=1` and 1/1 under UndefinedBehaviorSanitizer with halt-on-error; neither reported a finding. The UBSan build excludes only `vptr` for the existing cross-DSO RTTI/link constraint. The first LSan invocation inside the sandbox was gated by the sanitizer's ptrace restriction; the identical unrestricted invocation is the reported passing result.
