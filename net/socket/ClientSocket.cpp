...
ClientSocket* ClientSocket::connect(const InetAddress& ina, std::function<void
(Request& req)> callback) { int csFd = ::socket(AF_INET, SOCK_STREAM, 0);

    int ret = ::connect(csFd, (struct sockaddr*) &(ina.getSockAddr()),
sizeof(struct sockaddr_in));
    ...
    if (ret == 0) {
        ...
        if (getsockname(csFd, (struct sockaddr*) &localAddress, &addressLength)
< 0) { delete cs; return 0;
        }
